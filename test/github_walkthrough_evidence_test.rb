...
class GitHubWalkthroughEvidenceTest < Minitest::Test
  ...
  def test_walkthrough_without_a_commit_pinned_changed_file_link_is_refused
    github = client(snapshot_response, described, files_response)
  ...
  def test_walkthrough_pin_that_only_prefixes_a_changed_path_is_refused
    github = client(snapshot_response, described, files_response(['README.md']))
  ...
  def test_walkthrough_pin_to_a_renamed_file_previous_path_is_accepted
    ...
    github = client(snapshot_response, described, files, *gate_responses, html_response,
  ...
  def test_walkthrough_omitting_a_completed_review_gate_is_refused
    github = client(snapshot_response, described, files_response, *gate_responses)
  ...
  def test_pending_review_checks_are_not_required_in_the_walkthrough
    ...
    github = client(snapshot_response, described, files_response, *pending_review_gate_responses, html_response,
  ...
  def test_walkthrough_fails_when_required_check_evidence_is_unavailable
    github = client(snapshot_response, described, files_response, response({ 'message' => 'error' }))
  ...
  def test_walkthrough_requires_a_skipped_required_check
    ...
    github = client(snapshot_response, described, files_response, response(skipped), response(skipped))
  ...
  def test_walkthrough_pin_on_a_later_files_page_is_accepted
    ...
    github = client(snapshot_response, described, files_response(page_one), files_response,
                    *gate_responses, html_response, review_response, review_response, snapshot_response)
    published = github.walkthrough(head: HEAD, body: WALKTHROUGH)
    assert_equal 'COMMENTED', published['state']
  end
  ...
  def test_walkthrough_pin_to_a_path_with_a_plus_sign_is_accepted
    ...
    github = client(snapshot_response, described, files_response([name]), *gate_responses, html_response,
                    review_response(body: body), review_response(body: body), snapshot_response)
    published = github.walkthrough(head: HEAD, body: body)
    assert_equal 'COMMENTED', published['state']
  end
  ...
  def test_walkthrough_requires_a_cancelled_required_check
    ...
    github = client(snapshot_response, described, files_response, response(cancelled), response(cancelled))
    error = assert_raises(Shaka::Error) { github.walkthrough(head: HEAD, body: "See #{PINNED_LINK}.") }
    assert_includes error.message, 'validate'
  end
end
