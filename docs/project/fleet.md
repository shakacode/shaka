...
| `recovery.workspace_path` | `recovery.publish_locations` | Preserve the existing boolean. It controls publication of both the checkout path and session link. |
...
3. Do not run `shaka seam init` over the existing seam: the initializer is for a new
   ...
   [ordered migration](../reference/configuration.md#standard-command-scripts): keep repository-owned
