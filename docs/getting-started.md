...
`--ci-review-agent` unless the policy is `none`. Repeat that flag for another CI job. Use `--plan` for an existing
...
`--review-policy`, `--ci-review-agent`, and `--merge-preference` only when the old file
