...
`review.local_reviewers` in the repository's trusted `.agents/agent-workflow.yml` lists the local
...
top-level `review.github_action_check` names the GitHub review job to read, separately from this list. That job
...
`review.github_action_check`. Under `thorough`, wait for that named check on the current head anyway.
...
| `thorough` | Also wait for a verified `review.github_action_check` on the current head. | Refuses `UNSTABLE`. Queue-disabled: `CLEAN` only. Queue-enabled: `CLEAN`, `BEHIND`, or `BLOCKED`. |
...
2. Restore review and finish bullets that always wait for `review.github_action_check` if you remove the
   key entirely.
...
- a verified `review.github_action_check` report for that SHA
...
ran, wait for **one** verified `review.github_action_check` report on the first ready-for-review push of the
task. Do not wait for that check again after a nit-only or diagnostic-only follow-up SHA.

Under `thorough`, wait for a verified `review.github_action_check` report on the current head, even after a
