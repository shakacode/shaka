...
before starting the next under the same Ask/Auto workflow. Native stacks remain
outside this pilot.
...
Include the check table, provenance, usage tables, and recovery note while unfinished.
...
Repository writing preferences belong in `AGENTS.md`; current task instructions
can refine them. No new style schema or prose score is needed. Self-edit the
content JSON; do not rewrite the rendered GitHub body or invoke a separate
rewriting skill for Shaka's publication step.
...
| Which public comment bodies reach the agent | [Public-comment reader](public-comments.md) |
...
accepted changes using the [pilot criteria](../pilot-plan.md#success-evidence-and-commit-attribution).
