...
| R5 | Avoid redundant merge decisions. | Ask hands a ready PR to GitHub's merge control after the walkthrough and required gates when nothing else remains, so the user can merge and archive without another agent turn. Auto submits an eligible ordinary change after the same gates, including required native approvals, without another question. A queue-enabled base uses GitHub Merge Queue; a queue-disabled base merges immediately. Unclear authority or risky changes need a human decision. Native stacks and user-armed auto-merge are outside scope. |
