...
| R19 | Keep Shaka product gaps actionable. | Unless skipped, an independently verified Shaka product gap outside the active request gets a cited public-source draft offered by the root task owner. Search the fixed public `shakacode/shaka` repository only after explicit search approval; share candidate links and metadata for user inspection. File the exact draft only after separate explicit approval. Keep private context out, and stop on repository mismatch, search failure or limit, or unresolved candidates. This flow offers issue creation; it never creates automatically. See the [issue-offer procedure](../skills/shaka/docs/shaka-issue-offer.md). |
