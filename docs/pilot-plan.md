...
| R16 | Recover an unfinished PR without its conversation. | While a PR has not reached its outcome, its description keeps a [recovery note](working-with-your-agent.md#recover-an-unfinished-pr) with owner, task, last observed activity, revision, state, and next action, and removes it at the outcome. Publish only safe aliases and shareable locators. A fresh task takes over only after the maintainer confirms the previous task stopped or is handing over, then rechecks live state, preserves reachable local work, and marks unreachable local work UNKNOWN. Each new owner publishes a random tag before recovering local work; a resuming task stops when the live note names another owner or tag. The note grants no authority; the maintainer's confirmation, not the note, prevents two writers. No heartbeat, lease, or coordination service. |
