...
| R5 | Avoid redundant merge decisions. | Ask hands a ready PR to GitHub's currently offered merge control after the walkthrough and required gates when nothing else remains, naming the expected head so the user can merge that revision and archive without another agent turn. Auto submits an eligible ordinary change after the same gates, including required native approvals, without another question. Auto uses GitHub Merge Queue when the base has a queue and waits for its terminal result; Ask leaves that click and any later queue failure to GitHub and a new task. Unclear authority or risky changes need a human decision. Native stacks and user-armed auto-merge are outside scope. |
...
| R18 | Always have a reviewer available. | What makes a review adversarial is the context, not the model: a fresh session that did not produce the change is a valid reviewer, including one running the implementation model. `review.local_review_agents` lists local review agents in preference order and `shaka reviewer` prefers a provider that did not implement the change, moving past anything evidenced unavailable. With nothing listed available, the implementation model in a fresh context remains the local option even if its CLI path failed. `shaka review run` checks documented CLI execution and a head-bound report; `shaka review check` labels fresh-host report evidence separately or returns a nonzero result with the reason no review completed. Each reviewer and revision is recorded in the chat and the PR review status line. See [review](review.md#choose-a-local-reviewer). |
| R19 | Keep Shaka product gaps actionable. | Unless skipped, an independently verified Shaka product gap outside the active request gets a cited public-source draft offered by the root task owner. Verify and search only the fixed public GitHub target `github.com/shakacode/shaka`, after exact-query approval; share candidate links and metadata for user inspection. After separate filing approval, repeat the metadata-only search; new or changed candidate metadata requires review and renewed approval. File the unchanged exact draft only after current candidates are cleared. Keep private context out, and stop on repository mismatch, search failure or limit, or unresolved candidates. This flow offers issue creation; it never creates automatically. See the [issue-offer procedure](../skills/shaka/docs/shaka-issue-offer.md). |
