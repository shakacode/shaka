...
module Shaka
  module Writing
    ...
    class Siblings
      ...
      # This workflow opens a COMMENT review only to publish a walkthrough, so the newest
      # one its own account wrote is the walkthrough a description now sits beside. Reading
      # the author and the review state rather than the rendered title keeps a walkthrough
      # supplied as raw Markdown in scope, and anyone may review a public pull request, so
      # another author's review never counts. None exists before the first walkthrough is
      # published, which is the one case that skips.
      ...
      def walkthrough?(review)
        review['state'] == 'COMMENTED' && review.dig('user', 'login') == @github.viewer
      end
    end
  end
end
